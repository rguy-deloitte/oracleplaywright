...
// Configuration
const excelFilePath = path.join(__dirname, '..', '..', 'excel-data-files', 'post-excel-batch.xlsx');
test.describe.configure({ mode: 'serial' });  


test.describe('Post Manual Journals with Data Loaded from Excel File', () => {
  ...
  allBatches.forEach((batchData) => {
    test(`Journal Batch ${batchData['Journal Batch']}`, async ({ page }, testInfo) => {
        ...
        await journals.postJournalBatch(page, testInfo);
